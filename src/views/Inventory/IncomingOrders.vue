<template>
  <div class="inventory-incoming animate-slide-up">
    <div class="header-section mb-6 flex justify-between items-center">
      <div>
        <h2 class="text-2xl font-bold">{{ $t('nav.incomingOrders') }}</h2>
        <p class="text-secondary mt-1">بدء التجهيز وخصم الكميات من المخزون</p>
      </div>
      <span class="badge badge-warning text-lg px-4 py-2">{{ orders.length }} وارد</span>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="order in orders" :key="order.id" class="glass-card flex flex-col justify-between border-t-4 border-[var(--warning)]">
        <div>
          ...
          <div class="mb-4">
            <p class="text-sm text-secondary mb-1">العميل</p>
            <p class="font-bold">{{ order.client?.name || '-' }}</p>
          </div>
          
          <div class="bg-[var(--bg-secondary)] p-3 rounded-lg text-sm mb-4">
            <p class="font-bold mb-2 border-b border-[var(--border-color)] pb-1">المنتجات المطلوبة</p>
        ...
        <button @click="startProcessing(order.id)" class="btn btn-primary w-full py-3" :disabled="actionLoading[order.id]">
      ...
      <div v-if="orders.length === 0" class="col-span-full text-center mt-12 text-secondary glass-card p-12">
        <Inbox :size="48" class="mx-auto mb-4 text-warning opacity-50" />
        <p class="text-xl">لا توجد طلبات واردة حالياً.</p>
      </div>
    </div>
  </div>
</template>
...
const fetchOrders = async () => {
  try {
    const response = await omsApi.get('/api/orders?status=pending_inventory_approval')
...
const startProcessing = async (orderId) => {
  ...
  try {
    await omsApi.post(`/api/orders/${orderId}/transition`, {
      targetStatus: 'processing_in_warehouse'
    })
    await fetchOrders()
  } catch (error) {
