<template>
  <div class="inventory-ready animate-slide-up">
    <div class="header-section mb-6 flex justify-between items-center">
      <div>
        <h2 class="text-2xl font-bold">{{ $t('nav.readyOrders') }}</h2>
        <p class="text-secondary mt-1">تأكيد جاهزية الطلبات ليتم تسليمها للسائقين</p>
      </div>
      <span class="badge badge-primary text-lg px-4 py-2">{{ orders.length }} جاهز للتحميل</span>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="order in orders" :key="order.id" class="glass-card flex flex-col justify-between border-t-4 border-[var(--primary)]">
        <div>
        ...
        <button @click="markAsReady(order.id)" class="btn btn-primary w-full py-3" :disabled="actionLoading[order.id]">
          <span v-if="actionLoading[order.id]" class="loader small mr-2 inline-block"></span>
          تأكيد الجاهزية (Ready for Pickup)
        </button>
      </div>

      <div v-if="orders.length === 0" class="col-span-full text-center mt-12 text-secondary glass-card p-12">
        <CheckSquare :size="48" class="mx-auto mb-4 text-primary opacity-50" />
        <p class="text-xl">لا يوجد طلبات تنتظر التأكيد.</p>
      </div>
    </div>
  </div>
</template>
...
import { CheckSquare, Truck } from 'lucide-vue-next'
...
const fetchOrders = async () => {
  try {
    const response = await omsApi.get('/api/orders?status=assigned_to_driver')
